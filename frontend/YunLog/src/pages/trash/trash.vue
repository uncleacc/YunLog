      ...
      <view class="diary-item" v-for="item in trashList" :key="item.id">
        ...
        <view class="diary-attachments" v-if="item.attachments && item.attachments.length > 0">
          <template v-for="(att, index) in item.attachments.slice(0, 3)" :key="index">
            <image
              v-if="att && att.url && isImageFile(att.url)"
          ...
          <!-- 非图片附件显示为文件图标 -->
          <template v-for="(att, index) in item.attachments.slice(0, 3)" :key="'file-' + index">
            <view 
              v-if="att && att.url && !isImageFile(att.url)"
...
export default {
  ...
  methods: {
    async LoadTrashList() {
      try {
        this.loading = true
        const result = await api.getTrashList({ page: this.currentPage, limit: 20 })
        this.trashList = result.list || []
        this.hasMore = result.hasNext || false
        
        // 为每个日记加载附件
        for (let diary of this.trashList) {
          try {
            const attachments = await api.getAttachmentsByDiary(diary.id)
            // 过滤掉无效的附件对象，确保每个附件都有 url 属性
            diary.attachments = (attachments || []).filter(att => att && att.url)
          } catch (error) {
            console.warn(`加载日记 ${diary.id} 的附件失败:`, error)
            diary.attachments = []
          }
        }
      } catch (error) {
    ...
    GetDaysLeft(deletedTime) {
      try {
        const now = new Date()
        const deletedDate = parseDate(deletedTime)
        const diffDays = Math.ceil((now - deletedDate) / (1000 * 60 * 60 * 24))
        return Math.max(0, 30 - diffDays)
      } catch (error) {
        console.error('GetDaysLeft - 日期计算失败:', error, deletedTime)
        return 0
      }
    },
    ...
    async PermanentDelete(id) {
      try {
        await api.permanentDeleteDiary(id)
        uni.showToast({
          title: '已永久删除',
          icon: 'success',
        })
    ...
    PreviewAttachment(attachment, index, item) {
      ...
      if (isImageFile(attachment.url)) {
        ...
        const imageUrls = item.attachments
          .filter(att => att && att.url && isImageFile(att.url))
          .map(att => att.url)
